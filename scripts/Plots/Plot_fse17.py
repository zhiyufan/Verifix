...
def summary_gold(df, pids=None):
    ...
    if pids is None or len(pids) > 1:
        return ['All', '-', total, rps]
    return [lab_name, pids[0], total, rps]
...
def get_dfs(lab_ids, pids):
    path = CF.path_itsp + 'result/'
    df_gold, df_verifix, df_clara = None, None, None
    import os
    for lab_id in lab_ids:
        # lab_num = lab_id.split('-')[1]
        # df_gold_temp = pd.read_excel('{}gold_patch_size_lab{}.xlsx'.format(path, lab_num))
        for assign_id in pids:
            ...
            # df_gold = df_gold_temp
                df_verifix, df_clara = df_verifix_temp, df_clara_temp
            else:
            # df_gold = df_gold.append(df_gold_temp)
                df_verifix = df_verifix.append(df_verifix_temp)
                df_clara = df_clara.append(df_clara_temp)

    # df_clara['clara_patch_size'] = df_clara['patch_size']
    # df_gold['relative_patch_size'] = df_gold['rps']
